...
func testRolesSoftDelete(t *testing.T) {
	t.Parallel()
...
func testRolesQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Roles().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testRolesSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testRolesQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Roles().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testRoleToManyUserHasCourses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadUserHasCourses(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyAddOpUserHasCourses(t *testing.T) {
	var err error
...
var (
	roleDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`, `DisplayName`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `DeletedAt`: `timestamp`}
	_           = bytes.MinRead
)
