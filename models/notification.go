...
type Notification struct {
	...
	// A URL that links the notification to the proper page.
	URL null.String `boil:"url" json:"url,omitempty" toml:"url" yaml:"url,omitempty"`
	// The user that received this notification.
	UserToID int `boil:"user_to_id" json:"user_to_id" toml:"user_to_id" yaml:"user_to_id"`
	// The time this notification was read.
	TimeRead null.Time `boil:"time_read" json:"time_read,omitempty" toml:"time_read" yaml:"time_read,omitempty"`
...
var NotificationTableColumns = struct {
	...
	UserToID:  "notification.user_to_id",
	TimeRead:  "notification.time_read",
...
var NotificationWhere = struct {
	...
	UserToID  whereHelperint
	TimeRead  whereHelpernull_Time
	...
	Body:      whereHelpernull_String{field: "`notification`.`body`"},
	URL:       whereHelpernull_String{field: "`notification`.`url`"},
	UserToID:  whereHelperint{field: "`notification`.`user_to_id`"},
	TimeRead:  whereHelpernull_Time{field: "`notification`.`time_read`"},
	CreatedAt: whereHelpertime_Time{field: "`notification`.`created_at`"},
	UpdatedAt: whereHelpernull_Time{field: "`notification`.`updated_at`"},
	DeletedAt: whereHelpernull_Time{field: "`notification`.`deleted_at`"},
}
...
type notificationR struct {
	UserTo *User `boil:"UserTo" json:"UserTo" toml:"UserTo" yaml:"UserTo"`
}
...
func (r *notificationR) GetUserTo() *User {
	if r == nil {
		return nil
	}
	return r.UserTo
...
var (
	notificationAllColumns            = []string{"id", "title", "body", "url", "user_to_id", "time_read", "created_at", "updated_at", "deleted_at"}
	notificationColumnsWithoutDefault = []string{"title", "body", "url", "user_to_id", "time_read", "updated_at", "deleted_at"}
...
// UserTo pointed to by the foreign key.
func (o *Notification) UserTo(mods ...qm.QueryMod) userQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`id` = ?", o.UserToID),
	}
...
// LoadUserTo allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (notificationL) LoadUserTo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeNotification interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args = append(args, object.UserToID)
	...
	if singular {
		...
		object.R.UserTo = foreign
		if foreign.R == nil {
			foreign.R = &userR{}
		}
		foreign.R.UserToNotifications = append(foreign.R.UserToNotifications, object)
		return nil
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserToID == foreign.ID {
				local.R.UserTo = foreign
				if foreign.R == nil {
					foreign.R = &userR{}
				}
				foreign.R.UserToNotifications = append(foreign.R.UserToNotifications, local)
				break
			}
		}
	}
...
// SetUserTo of the notification to the related item.
// Sets o.R.UserTo to related.
// Adds o to related.R.UserToNotifications.
func (o *Notification) SetUserTo(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &userR{
			UserToNotifications: NotificationSlice{o},
		}
	} else {
		related.R.UserToNotifications = append(related.R.UserToNotifications, o)
	}
...
func Notifications(mods ...qm.QueryMod) notificationQuery {
	mods = append(mods, qm.From("`notification`"), qmhelper.WhereIsNull("`notification`.`deleted_at`"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`notification`.*"})
	}
...
func FindNotification(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Notification, error) {
	...
	query := fmt.Sprintf(
		"select %s from `notification` where `id`=? and `deleted_at` is null", sel,
	)
...
func (o *Notification) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE `notification` SET %s WHERE `id`=?",
			strmangle.SetParamNames("`", "`", 0, wl),
		)
...
func (q notificationQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o NotificationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE `notification` SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, notificationPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("`", "`", 0, wl),
		)
...
func NotificationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `notification` where `id`=? and `deleted_at` is null limit 1)"
