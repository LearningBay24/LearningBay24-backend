...
func testLanguageToManyPreferredLanguageUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.PreferredLanguageID = a.ID
	c.PreferredLanguageID = a.ID
	...
	check, err := a.PreferredLanguageUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.PreferredLanguageID == b.PreferredLanguageID {
			bFound = true
		}
		if v.PreferredLanguageID == c.PreferredLanguageID {
			cFound = true
		}
	}
	...
	if err = a.L.LoadPreferredLanguageUsers(ctx, tx, false, (*[]*Language)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PreferredLanguageUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.PreferredLanguageUsers = nil
	if err = a.L.LoadPreferredLanguageUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PreferredLanguageUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testLanguageToManyAddOpPreferredLanguageUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddPreferredLanguageUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.PreferredLanguageID {
			t.Error("foreign key was wrong value", a.ID, first.PreferredLanguageID)
		}
		if a.ID != second.PreferredLanguageID {
			t.Error("foreign key was wrong value", a.ID, second.PreferredLanguageID)
		}

		if first.R.PreferredLanguage != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.PreferredLanguage != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.PreferredLanguageUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.PreferredLanguageUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.PreferredLanguageUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	languageDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`}
	_               = bytes.MinRead
)
