...
type Language struct { // 2-letter ISO 639-1 language code.
...
var LanguageRels = struct {
	PreferredLanguageUsers string
}{
	PreferredLanguageUsers: "PreferredLanguageUsers",
}

// languageR is where relationships are stored.
type languageR struct {
	PreferredLanguageUsers UserSlice `boil:"PreferredLanguageUsers" json:"PreferredLanguageUsers" toml:"PreferredLanguageUsers" yaml:"PreferredLanguageUsers"`
}
...
func (r *languageR) GetPreferredLanguageUsers() UserSlice {
	if r == nil {
		return nil
	}
	return r.PreferredLanguageUsers
}
...
// PreferredLanguageUsers retrieves all the user's Users with an executor via preferred_language_id column.
func (o *Language) PreferredLanguageUsers(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`user`.`preferred_language_id`=?", o.ID),
	)

	return Users(queryMods...)
}

// LoadPreferredLanguageUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (languageL) LoadPreferredLanguageUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLanguage interface{}, mods queries.Applicator) error {
	var slice []*Language
	var object *Language
	...
	query := NewQuery(
		qm.From(`user`),
		qm.WhereIn(`user.preferred_language_id in ?`, args...),
	...
	if singular {
		object.R.PreferredLanguageUsers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userR{}
			}
			foreign.R.PreferredLanguage = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.PreferredLanguageID {
				local.R.PreferredLanguageUsers = append(local.R.PreferredLanguageUsers, foreign)
				if foreign.R == nil {
					foreign.R = &userR{}
				}
				foreign.R.PreferredLanguage = local
				break
			}
		}
	}
...
// AddPreferredLanguageUsers adds the given related objects to the existing relationships
// of the language, optionally inserting them as new records.
// Appends related to o.R.PreferredLanguageUsers.
// Sets related.R.PreferredLanguage appropriately.
func (o *Language) AddPreferredLanguageUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.PreferredLanguageID = o.ID
	...
	if o.R == nil {
		o.R = &languageR{
			PreferredLanguageUsers: related,
		}
	} else {
		o.R.PreferredLanguageUsers = append(o.R.PreferredLanguageUsers, related...)
	}
...
func FindLanguage(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Language, error) {
	...
	query := fmt.Sprintf(
		"select %s from `language` where `id`=?", sel,
	)
...
func LanguageExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `language` where `id`=? limit 1)"
